<template>
  <div class="container">
    ...
    <LoadMore :allLoaded="allLoaded" :loading="loading" @on-refresh="refresh" @on-loadmore="loadmore">
      <ul class="courses">
        <li v-for="course in courses" :key="course.id" class="micro-course" @click="toDetail(course.id)">
          ...
          <div class="course-info">
            ...
            <div class="flex">
              ...
              <span class="cancel" @click.stop="cancelCollect(course.id)">取消收藏</span>
            </div>
          </div>
        </li>
      </ul>
  ...
  methods: {
    getCourses (pageNo) {
      return new Promise((resolve) => {
        this.axios.get(`micclass/collects.do`, {
          params: {
    ...
    cancelCollect (id) {
      this.messageBox.confirm('确定要取消收藏？', '取消收藏').then(action => {
        if (action === 'confirm') {
          this.doCancel(id)
        }
      }, cancel => {})
